...
Up creates a private HTTPS certificate on first launch. Your browser will warn that it is not issued by a public certificate authority; verify that the address is your computer's private LAN address before accepting it. Pairing redirects the phone to a persistent, revocable device link that remains valid across app restarts. Treat that link like a password and do not share it.
